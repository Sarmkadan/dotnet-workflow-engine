...
The `AuditServiceExtensions` class provides extension methods for the `AuditService` that add comprehensive audit log querying and analysis capabilities. These extensions enable filtering audit logs by workflow, severity, date range, actor, and activity, as well as calculating workflow durations, failure rates, and generating CSV exports.

The extension methods provide a fluent, LINQ-friendly API for querying audit data without needing to access the underlying repository directly, making it easier to build monitoring dashboards, compliance reports, and debugging tools.
...
// Example 1: Get audit logs for a specific workflow with pagination
var (workflowLogs, totalCount) = await auditService.GetWorkflowAuditLogAsync(
    workflowId: "order-processing-workflow",
    skip: 0,
    take: 50
);
Console.WriteLine($"Workflow '{workflowLogs.FirstOrDefault()?.WorkflowId}' has {workflowLogs.Count} audit entries (total: {totalCount})");

// Example 2: Filter audit logs by severity level
var (errorLogs, errorCount) = await auditService.GetBySeverityAsync(
    severity: "Error",
    skip: 0,
    take: 100
);
Console.WriteLine($"Found {errorLogs.Count} error entries out of {errorCount} total");

// Example 3: Filter audit logs by date range
var (recentLogs, recentCount) = await auditService.GetByDateRangeAsync(
    fromDate: DateTime.UtcNow.AddDays(-7),
    toDate: DateTime.UtcNow,
    skip: 0,
    take: 25
);
Console.WriteLine($"Found {recentLogs.Count} entries from the last 7 days");

// Example 4: Get audit logs for a specific actor
var (userLogs, userCount) = await auditService.GetByActorAsync(
    actor: "admin@company.com",
    skip: 0,
    take: 25
);
Console.WriteLine($"User 'admin@company.com' performed {userCount} actions");

// Example 5: Get recent audit logs across all workflows
var globalRecentLogs = await auditService.GetGlobalRecentAuditLogAsync(count: 10);
Console.WriteLine($"Most recent 10 entries:");
foreach (var log in globalRecentLogs.Take(3))
{
    Console.WriteLine($"  - {log.Timestamp:yyyy-MM-dd HH:mm:ss} {log.EventType}: {log.Description}");
}

// Example 6: Get workflow duration statistics
var (startTime, endTime, durationMs) = await auditService.GetWorkflowDurationAsync(
    instanceId: "wf-order-processing-001"
);
Console.WriteLine($"Workflow ran from {startTime:HH:mm:ss} to {endTime?.ToString("HH:mm:ss") ?? "running"} ({durationMs}ms)");

// Example 7: Get activity failure rate
var (totalAttempts, failures, failureRate) = await auditService.GetActivityFailureRateAsync(
    activityType: "validate-order",
    timeWindowDays: 30
);
Console.WriteLine($"Activity 'validate-order': {totalAttempts} attempts, {failures} failures ({failureRate}% failure rate)");

// Example 8: Export audit logs as CSV
var csvExport = await auditService.ExportAuditLogAsCsvAsync(
    instanceId: "wf-order-processing-001",
    fields: new List<string> { "Timestamp", "EventType", "ActivityId", "Severity", "Description", "Actor" }
);
Console.WriteLine("CSV Export:");
Console.WriteLine(csvExport.Substring(0, Math.Min(200, csvExport.Length)) + "...");

// Example 9: Get instance state summary
var stateSummary = await auditService.GetInstanceStateSummaryAsync(
    instanceId: "wf-order-processing-001"
);
Console.WriteLine("Instance state summary:");
foreach (var kvp in stateSummary)
{
